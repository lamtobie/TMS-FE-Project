...
import { DeliveryOrderService } from "app/modules/deliveryorder/deliveryorder.service";
...
@Component({
    selector     : 'getrefund',
    templateUrl  : './getrefund.component.html',
...
export class GetRefund
{
    ...
    constructor(
        ...
        private dialogRef: MatDialogRef<GetRefund>,
  ...
  ngOnInit(): void {
    ...
    getOrderByCode(){
        this.sessionService.getOneByCode(this.orderCode.orderCode).subscribe({next : (data) => {
            if (data) {
    ...
    refund(){
        var note=this.supportForm.get('reason').value;
                this.orderConfirm.reasonCancel=note;
                console.log(this.session);
                this.sessionService.returnedSession(this.session.code,this.orderConfirm).subscribe({
                    next: (result) => {
                        if (result.success == true) {
                            const confirmation = this._fuseAlertnService.open({
                                title  : 'Success',
                                message: 'Get refund successfully for session '+this.session.code,
                                icon       : {
                                    show : true,
                            ...
                            const confirmation = this._fuseAlertnService.open({
                                title  : 'Failed',
                                message: 'Get refund failed for session '+this.session.code,
                                icon       : {
                                    show : true,
